...
from bringyourownproxies.captcha import (DEFAULT_CAPTCHA_SOLVER, DEFAULT_CAPTCHA_MAXIMUM_WAITING,
                                         DeathByCaptcha,DeathByCaptchaProblem,
                                         CaptchaProblem)
...
class _Account(OnlineAccount):
    ...
    @staticmethod
    def submit_captcha_and_wait(captcha,
        ...
        def poll_death_by_captcha(captcha_id):
            ...
            check_status = requests.get(url,headers={'Accept':'application/json'})
        ...
        if isinstance(captcha_solver,DeathByCaptcha):
            ...
            if 'error' in response:
                raise DeathByCaptchaProblem('Error while submitting '\
                                            'captcha:{error}'.format(error=response['error']))
                ...
                if response['is_correct']:
                    if not response['text']:
                        ...
                        if not captcha_response:
                            raise DeathByCaptchaProblem('Timed out, took more than '\
                                                        'than the maximum amount of '\
                                                        'time:{t} allowed to retrieve a '\
                                                        'response'.format(t=maximum_waiting_time))
                    else:
                        captcha_response =  response['text']
                else:
                    raise DeathByCaptchaProblem('Captcha is incorrect')
            else:
                raise DeathByCaptchaProblem('Unknown response by deathbycaptcha')
...
if __name__ == '__main__':
    ...
    response = _Account.submit_captcha_and_wait('/root/Dropbox/recaptcha.png',captcha_solver,180)
    print response
