...
class YouPornUpload(_Upload):
    ...
    def update_video_settings(settings, video_id, account):
        ...
        post = {
            "videoedit[title]": settings['title'],
        ...
        if 'success' in response:
            ...
            raise FailedUpdatingVideoSettings(
                'Unknown status:{s}'.format(
                    s=update_settings.content))
    ...
    def get_thumb_nails(video_id, account):
        ...
        go_to_thumbnails = session.get(
            'http://www.youporn.com/myuploads/edit/thumbnails',
            proxies=proxy)
