...
from bringyourownproxies.errors import (InvalidVideoUploadRequest,InvalidAccount,NotLogined,
                                        InvalidTitle,FailedUpload,CannotFindVar)
...
class PornhubUpload(_Upload):
    ...
    def start(self):
        ...
        try:
            ...
            if len(get_cookie) == 0:
                raise CannotFindVar('Cannot find input field with name:cookies[]')
            if len(get_platform_id) == 0:
                raise CannotFindVar('Cannot find input field with name:platformId[]')
            if len(get_source) == 0:
                raise CannotFindVar('Cannot find input field with name:source[]')
            ...
            is_straight = self.video_upload_request.is_straight
            ...
            if not isinstance(category,(tuple,list)):
                ...
                categories = ['"{c}"'.format(c=str(cat.category_id)) for cat in category]
            ...
            get_upload_url = re.search(r"var\s+url\s+=\s+'(.*?)',",go_to_upload.content)
            ...
            with open('upload_video.html','w+') as f:
                f.write(upload_video.content)
