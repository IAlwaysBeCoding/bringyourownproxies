...
class HttpSettings(object):
    ...
    def build_proxy_settings(self):
        ...
        if ('username' not in self.proxy) or ('password' not in self.proxy):
            self._validate_proxy(proxy=self.proxy)
            proxy_string = 'http://{ip}:{port}'.format(ip=self.proxy['ip'],port=self.proxy['port'])
        else:
            proxy_string = 'http://{username}:{password}@{ip}:{port}'.format(username=self.proxy['username'],
